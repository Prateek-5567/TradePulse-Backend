...
import com.prateek.request.LoginRequest;
...
public class AuthController {
	...
	public ResponseEntity<AuthResponse> createUserHandler(
		...
		watchlistService.createWatchList(savedUser); // create watchlist service for new user.
//		walletService.createWallet(user);
	...
	public ResponseEntity<AuthResponse> signing(@RequestBody LoginRequest loginRequest) throws UserException, MessagingException {
		...
		if(user.getTwoFactorAuth().isEnabled()){
			...
			TwoFactorOTP twoFactorOTP=twoFactorOtpService.createTwoFactorOtpRecordInTable(user,otp,token);
	...
	private Authentication authenticate(String username, String password) {
        ...
        SecurityContextHolder
            SecurityContextHolder.getContext().setAuthentication(authentication);
            Stores authenticated user for current request lifecycle.
            After this:
            Spring Security treats request as logged-in user
         */
	}


	@GetMapping("/login/google")   // ( to redirect the user to OAuthLogin )
    ...
    //	/login/oauth2/code/google ( to redirect from google auth provider page to you app )
...
// TwoFactorOTP is a class corresponding to database table that contain only two values : {id , otp }

		TwoFactorOTP otpInsideDb = twoFactorOtpService.findById(id);

		if(twoFactorOtpService.verifyTwoFactorOtp(otpInsideDb,otp)){
			AuthResponse authResponse = new AuthResponse();  // AuthResponse is a customised response object for authentications service.
			authResponse.setMessage("Two factor authentication verified");
			authResponse.setTwoFactorAuthEnabled(true);
			authResponse.setJwt(otpInsideDb.getJwt());
			return new ResponseEntity<>(authResponse, HttpStatus.OK);
		}
...
@RequestMapping("/auth")
Definition
@RequestMapping("/auth") is a Spring annotation used to define a base URL path for all request mappings within a controller class.
Conceptual Understanding
It acts as a prefix for all endpoints inside the controller. Any method-level mapping will be appended to this base path. 
eg you create a method with mapping /login/google but the mapping is actually  auth/login/google  
we need all auth request urls to start with /auth/**
